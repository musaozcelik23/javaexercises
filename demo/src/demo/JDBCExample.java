			...
			if (secim.equals(a)) {
				...
				if (secimno.equals(e)) {
					...
					rs = stmt.executeQuery(
							"SELECT c.id,c.name,s.name from channel c inner join station s on s.id=c.station_id ");
					while (rs.next()) {
					...
					rs = stmt.executeQuery(
							"SELECT c.name,s.name,m.value,m.measure_date from channel c inner join station s on s.id=c.station_id inner join measure m on m.channel_id=c.id ");
					while (rs.next()) {
				...
				stmt.executeUpdate("INSERT INTO measure (measure_date,value,channel_id) VALUES ( now()," + deger + ","
						+ kanal + ")");
				System.out.println("kayıt edildi");
